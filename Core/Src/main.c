	  ...
	  if (new_sample_ready)
	      {
	          ...
	          if (!paused)      // if PC13 pressed not pressed
	          {
	              ...
	              // UART TRANSMISSION
	              char uart_msg[64];
	              sprintf(uart_msg, "Filtered: %u,CCR:%u, PWM: %u%%\r\n", filtered_value, ccr_value,pwm_percentage );
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef *hadc)
{
    ...
    if (buffer_index >= WINDOW_SIZE)
    {
        ...
        buffer_full = 1;                         // we've now completed at least one full lap
    }

    new_sample_ready = 1;							// tell main new data available
}


void  HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
	if(GPIO_Pin == GPIO_PIN_13){ // external interrupt
		uint32_t current_press_time = HAL_GetTick();
		if ((current_press_time - last_press_time) > DEBOUNCE_MS)
		        {
		            paused = !paused;
